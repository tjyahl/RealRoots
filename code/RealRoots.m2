...
document {
	...
	Headline => "the k x k submatrix generated by the corresponding leading principal minor of the Hurwitz matrix H of a rational univariate polynomial",
	Usage => "HurwitzMatrix(f,k)",
	Inputs => {
	    RingElement => "f" => {"a rational univariate polynomial"},
	    ZZ => "k" => {"a nonnegative integer that determines the dimensions of a square submatrix of the Hurwitz matrix of ",TT "f","."},
	    },
	Outputs => {Matrix => {"the k x k submatrix ",TEX///$H_{k}$///," generated by the corresponding leading principal minor of the Hurwitz matrix ",TEX///$H$///," of ", TT "f","."}},
	PARA{"This computes the k x k submatrix ",TEX///$H_{k}$///," of the corresponding leading principal minor of the Hurwitz matrix ",TEX///$H$///," of a rational univariate polynomial ", TT "f"," with positive leading coefficient and degree at least one.
...
document {
	Key => {(HurwitzDeterminant, RingElement, ZZ),HurwitzDeterminant},
	Usage => "HurwitzDeterminant(f,k)",
	Inputs => {"f","k"},
	Outputs => { RR => { "the Hurwitz determinant of a real univariate polynomial", TT "f", "of order k"}},
	PARA {"This computes the Hurwitz determinant of a real univariate polynomial ", TT "f", "with positive leading coefficient and degree at least 1. The polynomial, however, is not necessarily from a ring with one indeterminate."},
	EXAMPLE lines ///
	    	R = QQ[x]
	        f = 3*x^4 - 7*x^3 +5*x - 7 
...
document {
	...
	Inputs => {"f"},
	Outputs => { Boolean => { "the Hurwitz stability of a real univariate polynomial", TT "f"}},
	PARA {"This determines the Hurwitz stability of a real univariate polynomial ", TT "f", "with positive leading coefficient and degree at least 1. The polynomial, however, is not necessarily from a ring with one indeterminate."},
	EXAMPLE lines ///
	    	R = QQ[x]
            	f = 3*x^4 - 7*x^3 +5*x - 7 
		g = x^2 + 10*x + 21
