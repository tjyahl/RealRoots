...
document {
	...
	PARA {"This computes the number of distinct real points of Spec ", TT "S", ", not counting multiplicity.
	       ...
	       where ",TEX///$M$/// ," is the ideal generated by ",TT "f" ," and all variables of ",TT "ring f" ," except the variable in ",TT "f",". If ", TT "l", " is a system of rational polynomials, then this computes its number of solutions."},
    	PARA {"This counts the number of real roots of a univariate polynomial ",TT "f",", without multiplicity."},
	EXAMPLE lines ///
 	         R = QQ[x,y]
		 f = (x^2 + 1)*(x + 1)*(x - 2)^2
		 realCount(f)
