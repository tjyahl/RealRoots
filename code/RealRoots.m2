...
document {
 ...
 document {
    ...
    --Sylvester count, check whether f and g are in the same ring, state in documentation
document {
...
document {
	...
	Inputs => {
	    ...
	    Ideal => "I" => {"the ideal generated by ", TT "g"},
	    },
...
document {
	...
	Inputs => {
	    QuotientRing => "R" => {"an Artinian ring"},
...
document {
    ...
    --principal k by k minor for outputs,(description) k by k leading principal minor, principal k by k submatrix v principal k by k minor
document {
	Key => {HurwitzMatrix,(HurwitzMatrix,RingElement),(HurwitzMatrix, RingElement, ZZ)},
	Headline => "a specified principal submatrix of the Hurwitz matrix of a univariate polynomial",
    ...
    --a univariate poly is hurwitz stable if each root has negative real part
document {
