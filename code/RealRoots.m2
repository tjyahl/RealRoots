...
document {
	...
	PARA {"If ",TT "f" ," is a univariate polynomial, this computes the number of distinct points of Spec ",TT "(ring f)/M"," where ",TEX///$M$///," is 
	    the ideal generated by ",TT "f"," and all the variables of ",TT "ring f"," except the variable of ",TT "f","."},
	EXAMPLE lines ///
 	         R = QQ[x,y]
		 f = (x^2 + 1)*(x + 1)*(x - 2)^2
		 traceCount(f)
...
document {
	...
	PARA {"This computes the number of distinct real points of Spec ", TT "S", ", not counting multiplicity, and where ", TT "S", " is an Artinian ring of characteristic 0.},
    	...
    	PARA {"If ", TT "f", " is a univariate polynomial, this computes the number of distinct real points of Spec ",TT "(ring f)/M", ",
	       where ",TEX///$M$/// ," is the ideal generated by ",TT "f" ," and all variables of ",TT "ring f" ," except the variable of ",TT "f","."},
	EXAMPLE lines ///
 	         R = QQ[x,y]
		 f = (x^2 + 1)*(x + 1)*(x - 2)^2
		 realCount(f)
		 ///,
	PARA {"If ",TT "I"," is a zero-dimensional ideal, this computes the number of distinct real points of Spec ",TT "(ring I)/I","."},
	EXAMPLE lines ///
		 I = ideal(5 - 3*x^2 - 3*y^2 + x^2*y^2, 1 + 2*x*y - 4*x*y^2 + 3*x^2*y)
		 realCount(I)
		 ///,
