...
%
  ...
  \]
%
the negative remainder term in the division of $f_{i-1}$ by $f_i$. \note{This is the Sylvester sequence of $f$ and $f'g$, not $f$ and $g$. Either correct this here or change the statement of the thoerem. Make sure the use is consistent throughout.}
...
and we also have that $g_k=1$. \note{Is there a better symbol than $g$ to use for the (reduced) Sylvester sequence?}
...
\end{leftbar}
%
These isolating intervals are shown in Figure~\ref{F:One}. \note{This should be above with the comments about isolation.}

A polynomial $f\in\RR[x]$ is \demph{Hurwitz-stable} if all of its complex roots have negative real parts.
