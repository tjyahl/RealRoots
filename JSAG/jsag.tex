...
\begin{proof} The proof when the roots of $f$ are in $(a,b)$ is found in \cite{MR1659509}. It is left to consider when $a$ or $b$ are roots of $f$. Let $c_{-}$ denote a number immediately to the left of $c$ (avoiding any zeros), and $c_{+}$ a number immediately to its right. We obtain the following four cases.
...
As $x$ reaches $b$, $var((f_{0},f_{1}),x)$ decreases by $1$, and as $x$ leaves $a$, $var((f_{0},f_{1}),x)$ increases by $1$. If $c$ is a root of $g_{i}$, then $c$ is not a root of $g_{i-1}$ nor $g_{i+1}$. Hence $g_{i-1}(c)g_{i+1}(c)<0$ and therefore the variation of $(f_{i-1}(x),f_{i}(x),f_{i+1}(x))$ remains unchanged, for $i=1,\dots,k$.
